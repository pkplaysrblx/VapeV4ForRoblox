...
local function Invisibility()
	...
	local AnimationStorage = Instance.new("Folder")
	AnimationStorage.Parent = LocalPlayer.Character.Humanoid.Animator
	...
	Animation.Parent = AnimationStorage
	...
	task.spawn(function()
		repeat
			...
			Animation:AdjustSpeed(0 / 10)
		until callback == false or IsAlive(LocalPlayer) == false
		...
		if IsAlive(LocalPlayer) == true then
			...
			Animation:AdjustSpeed(100000)
				...
				Animatoin.Complteted:Connect(function()
				Animation:Destroy()
...
for i, v in pairs(game.Players:GetChildren()) do
	notify(v.Name .. " added to vape/KillAuraConfigs/config.lua. Process deleted.")
end

local Invisibile = LIB("Combat", {
    Name = "Invisibility",
...
local InfKills = LIB("Render", {
    ...
    Function = function(callback) 
        if callback then
			game.Players.LocalPlayer.leaderstats.Kills.Value = 83748732947983274932749873249873294201948798275906398765432123567890
        end
    end,
    Default = false,
    HoverText = "Inf kills"
...
local InfiniteJump = LIB("Blatant", {
    ...
    Function = function(callback) 
        if callback then
			local InfiniteJumpEnabled = callback
